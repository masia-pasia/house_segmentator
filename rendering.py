from MONAI.segmentator import HouseSegmentator
import torch
...
X = config.TRANSFORM(paths[0]) # transform ścieżka -> obraz
X = X.unsqueeze(0) # jeśli jest tylko jeden obraz to musimy dołożyć kanał na batch

checkpoint = 'MONAI/lightning_logs/1024_5l_1drop_2Resnet_w_class_3_affine_aug/best_checkpoint-v1.ckpt'
model = UNet(
    spatial_dims=2,
    in_channels=3,
    out_channels=3,
